...
export const getProducts = async () => {
    const products = await fetch(`${process.env.NEXT_PUBLIC_API_URL}/products`, { cache: "no-cache" })
    return await products.json()
}

export const getProductDetails = async (productId: string) => {
    const productDetails = await fetch(`${process.env.NEXT_PUBLIC_API_URL}/products/${productId}`, { cache: "no-cache" })
...
export const getOrdersByCustomer = async (customerId: string) => {
    const ordersByCustomer = await fetch(`${process.env.NEXT_PUBLIC_API_URL}/orders/customers/${customerId}`, { cache: "no-cache" })
